...
local function table_merge(...)
    ...
    for index = 1, select("#", ...) do
        assert(type(arguments[index]) == "table",
            "Expected table-only argument, got " .. type(arguments[index]) .. " at index " .. index)
    end
...
local function parseClass(name, ...)
    ...
    for index = 1, select("#", ...) do
        ...
        assert(type(arguments[index]) == "table",
            "Non-table parameter passed to class " .. name .. " - " .. type(arguments[index]))
    end
